...
The platform generates structured event logs in JSONL (JSON Lines) format for analytics, auditing, and operational monitoring.
...
Each line in the file represents a single event encoded as a valid JSON object.
...
The event logging system captures visitor activity and store interactions, enabling:
...
The JSONL format was selected because it is lightweight, human-readable, easy to validate, and compatible with downstream analytics pipelines.

Event records are generated by the analytics pipeline and stored in an append-only format to support scalable event processing workflows.
